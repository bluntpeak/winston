...
exports.log = function (options) {
  ...
  //
  // Really ugly hack - should be easier to do this...
  // consolejson - alternative prettyprint mode that does not require or assume 
  // a first argument as a string message but can cope nice with a single 
  // argument as a JSON object to log (message and meta in one go)
  //

  if (options.consolejson) {
    var entry = {};
    if (typeof options.message === 'object') {
      entry = exports.clone(cycle.decycle(options.message));
      // and for now assume we do not send meta if we send obj as first 
      // i.e. message param to the log call
    } else {
      // assume we sent a string message and check for meta
      if (meta) {
        entry = meta;
        entry.message = options.message;
      } else {
        // just a single message string param we assume
        entry = {message: options.message, justMessage: true};
      }
    }

    // so we have an entry object - now log the bitch!
    var output = options.colorize ? config.colorize(options.level) : options.level;
    output += (': ' + entry.message);

    if (entry.justMessage)
      return output;
